...
begin
  for cc in select dta from subs loop
    ...
    -- В рамках ограниченного временного формата было принято решение остановиться на определённом этапе
    -- Дальнейший поиск вариантов исправления НЕВАЛИДНОГО JSON в исходных данных не позволил бы вычислить Рейтинг
      end;
  end loop;
end;
$$;
